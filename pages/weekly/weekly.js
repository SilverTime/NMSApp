...
var viewAllTopicInfor = function (that) {
  ...
  wx.request({
    method: 'POST',
    url: ajaxUrl.ajaxUrl() + 'abstractCategory/viewAbstract',
    data: {
      "pageArray": pageArray,
      "recordPerPage": 20,
    ...
    success: function (res) {
      ...
      if (res.data.ret == true) {
        ...
        if (getApp().globalData.weeklyFirst != 0) {//不是第一次请求，会产生重复数据，需要去重
          var len = parseInt(getApp().globalData.weeklyFirst) + 1;
        ...
        for (var k = 0; k < my_data.length;k++){
          for (var i = 0; i < my_data[k].data.length; i++) {
            if (my_data[k].data[i].abstractcState == 0) {
              my_data[k].data[i].abstractcStateName = '可编辑';
            }
            else {
              my_data[k].data[i].abstractcStateName = '已终稿';
            }
          }
        }
        ...
        getApp().globalData.weeklyFirst = getApp().globalData.weeklyFirst + 1;
      }
    },
    fail: function (res) {
      console.log(res);
...
Page({
  ...
  detailData: function (e) {
    ...
    if (!flag) {
      wx.navigateTo({
        url: '../abstractList/abstractList?abstractcId=' + e.target.dataset.abstractcid,
      })
    }
  },
  touchS: function (e) {
  ...
  delItem: function (e) {
    var that = this
    var abstractcId = e.target.dataset.id;
    if (abstractcId!=undefined){
      wx.showModal({
        title: '提示',
        content: '是否删除周报？',
        success: function (res) {
          if (res.confirm) {
            wx.request({
              method: 'POST',
              url: ajaxUrl.ajaxUrl() + 'abstractCategory/deleteAbstract',
              data: {

...
var viewAllCollectPage = function (that) {
  for (var i = 1; i <= 3; i++) {
    pageArray.push(i);//未指定页数，则只读取前三页数据
  }
  wx.request({
    method: 'POST',
    url: ajaxUrl.ajaxUrl() + 'collect/viewAllCollectPage',
    data: {
      "pageArray": pageArray,
      "recordPerPage": 20,
    },
      ...
      if (res.data.ret == true) {
        ...
        for (var k = 0; k < my_data.length; k++) {
          for (var i = 0; i < my_data[k].data.length; i++) {
            if (my_data[k].data[i].collectSensibilities < 0) {
              my_data[k].data[i].collectSenMessage = '负面';
            }
            else if (my_data[k].data[i].collectSensibilities > 0) {
              my_data[k].data[i].collectSenMessage = '正面';
            }
            else {
              my_data[k].data[i].collectSenMessage = '中性';
            }
          }
        }
...
Page({
  ...
  detailData: function (e) {
    ...
    var flag = false;
    //console.log(e.target.dataset.collectid);//当前点击的collectid
    for (var i = 0; i < list.length; i++) {
      //当前属于选中状态
    ...
    if (!flag) {
      wx.navigateTo({
        url: '../dataDetails/dataDetails?collectId=' + e.target.dataset.collectid,
      })
    }
  ...
  delItem: function (e) {
    var that = this
    var collectId = e.target.dataset.collect;
    var webId = e.target.dataset.webid;
    //console.log(webId);
    wx.showModal({
      title: '提示',
      content: '是否取消收藏？',
